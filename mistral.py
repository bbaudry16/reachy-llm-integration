...
class MistralClient:
    ...
    def _buildMessages(self, userMessage: str) -> list:
        messages = [{"role": "system", "content": self.systemPrompt}]
        messages.extend(self.history)
        messages.append({"role": "user", "content": userMessage})
        return messages

    def _updateHistory(self, userMessage: str, reply: str) -> None:
    ...
    def ask(self, userMessage: str) -> dict:
        self._waitRateLimit()
        headers = {"Authorization": f"Bearer {self.apiKey}", "Content-Type": "application/json"}
        payload = {
            "model": self.MODEL,
            "messages": self._buildMessages(userMessage),
        ...
        self._updateHistory(userMessage, result.get("speech", ""))
        return result
