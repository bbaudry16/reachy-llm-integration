...
STRATEGY 2 — BRIDGE BLOCKS: between two speak blocks, insert a silent movement block:
- parallel:
    - move_hand:
        arm: right
...
step_duration × number_of_positions = speech duration. Always verify.
step_duration min 0.4 — prefer 0.5-0.6 for natural fluid motion.
For set_antenna: duration 0.5-1.0. Never instant.
For vibrate_antenna: speed 0.12-0.20. Cycles 3-6.
NEVER let a block contain only speak_a_text and look_at — every block MUST have
at least one arm action AND one antenna action.
